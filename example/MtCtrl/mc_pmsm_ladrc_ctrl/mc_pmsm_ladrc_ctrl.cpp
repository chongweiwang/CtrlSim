...
#include "mc_pmsm_ladrc_ctrl.hpp"

McPmsmLadrcCtrl::McPmsmLadrcCtrl(SimObj *parent):SimObj(parent)
{
...
//    vel_ladrc.setClampParm(1,-1);
        ...
        double cw = 2000;
        double cb0 = 200*pmsm.cfg.nameplate.rated_vol;
        Iq_ladrc.setCtrlParm(cw,6*cw,cb0,0.00005);
        Iq_ladrc.setClampParm(0.5,-0.5);

        Id_ladrc.setCtrlParm(cw,6*cw,cb0,0.00005);
        Id_ladrc.setClampParm(0.5,-0.5);

        double vw = 500;
        double vb0 = 30*pmsm.cfg.nameplate.rated_cur;
        vel_ladrc.setCtrlParm(vw,6*vw,vb0,0.00005);
        vel_ladrc.setClampParm(1,-1);
}

McPmsmLadrcCtrl::~McPmsmLadrcCtrl()
{

}


void McPmsmLadrcCtrl::dynamic_ode(double *dx, double *x,double *u)
{
    Q_UNUSED(dx);
    Q_UNUSED(x);
    Q_UNUSED(u);
...
void McPmsmLadrcCtrl::sim_run(void)
{
    run();
}

void McPmsmLadrcCtrl::foc_cur_loop(void)
{
    ...
    foc.o_Vq_pu = Iq_ladrc.ladrc_1st(foc.i_ref_Iq_pu,foc.m_Iq_pu);
    foc.o_Vd_pu = Id_ladrc.ladrc_1st(foc.i_ref_Id_pu,foc.m_Id_pu);
...
void McPmsmLadrcCtrl::vel_loop(void)
{
    Vel.o_tor_ctrl_pu  = vel_ladrc.ladrc_1st(Vel.i_ref_rmp_pu,Vel.i_rpm_fb_pu);
    //Vel.o_tor_ctrl_pu =  vel_pid.PI(Vel.i_ref_rmp_pu - Vel.i_rpm_fb_pu);
}

void McPmsmLadrcCtrl::simulation(void)
{
...
void McPmsmLadrcCtrl::wavePlot(void)
{
